...
const ChecklistView = ({
  ...
  // 샘플 데이터에서 이벤트 찾기
  const event = propEvent || sampleEvents.find(e => String(e.id) === String(id));

  // 이벤트의 카테고리 데이터를 사용하거나 기본 데이터 사용
    ...
    if (event && event.categories) {
      ...
      return event.categories.map(category => ({
        ...
        items: category.items.map(item => ({
          ...item,
          subItems: [], // 샘플 데이터에는 subItems가 없으므로 빈 배열로 설정
          type: category.name === '당일 준비' ? '당일' : '사전'
        }))
      }));
    }
